...
module.exports = function (opts) {
    ...
    return through.obj(function (file, enc, cb) {
        ...
        if (file.isStream()) {
            cb(new pluginError('gulp-fez-fontmin', 'Streaming not supported'));
            return;
        }

        if (validExts.indexOf(path.extname(file.path).toLowerCase()) === -1) {
            if (opts.verbose) {
                fancyLog('gulp-fez-fontmin: Skipping unsupported font ' + chalk.blue(file.relative));
            }
        ...
        fontmin.run(function (err, files) {
            if (err) {
                cb(new pluginError('gulp-fez-fontmin:', err, {fileName: file.path}));
                return;
            }
